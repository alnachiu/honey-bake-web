  ...
  return (
    <div className="page-container pt-4 pb-20">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="card space-y-3">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {form.images.length === 0 && (
              <p className="text-xs text-text-light py-3">暂无主图，点击"本地上传主图"选择图片</p>
            )}
          </div>
        </div>
        <div className="card space-y-3">
        ...
        <button type="submit" disabled={saving || uploading} className="btn-primary w-full">{saving ? '保存中...' : '保存修改'}</button>
      </form>
    </div>
  )
}
